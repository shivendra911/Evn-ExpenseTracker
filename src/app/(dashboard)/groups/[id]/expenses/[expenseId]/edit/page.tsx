...
export default function EditGroupExpensePage() {
  ...
  const { data: expense, isLoading: expenseLoading } = useQuery({
    queryKey: ['groupExpense', groupId, expenseId],
    queryFn: () => getGroupExpense(groupId, expenseId),
  });
  ...
  useEffect(() => {
    if (expense && !initialized) {
      ...
      // Set the payer (first contributor)
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => updateGroupExpense(groupId, expenseId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['groupExpenses', groupId] });
      queryClient.invalidateQueries({ queryKey: ['groupBalances', groupId] });
      queryClient.invalidateQueries({ queryKey: ['groupExpense', groupId, expenseId] });
  ...
  // Authorization check on the client side
  if (expense && user && expense.createdById !== user.id) {
    return (
      <div className="card empty-state" style={{ padding: 48 }}>
        ...
        <Link href={`/groups/${groupId}`} className="btn btn-primary btn-sm" style={{ marginTop: 16 }}>
          Back to Group
        </Link>
      </div>
    );
