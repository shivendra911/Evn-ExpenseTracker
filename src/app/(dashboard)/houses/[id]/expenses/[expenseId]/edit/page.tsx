...
import { getGroupExpense, updateGroupExpense } from '@/api/endpoints/groupExpenses';
import { EXPENSE_CATEGORIES } from '@/shared/validation';
import type { ExpenseCategory, SplitType } from '@/shared/types';
import { rupeesToPaise, paiseToRupees, formatCurrency } from '@/lib/money';
import { useToast } from '@/components/ui/Toast';
import { useAuthStore } from '@/store/auth';

export default function EditHouseExpensePage() {
  ...
  const { data: expense, isLoading: expenseLoading } = useQuery({
    queryKey: ['groupExpense', houseId, expenseId],
    queryFn: () => getGroupExpense(houseId, expenseId),
  });
  ...
  useEffect(() => {
    if (expense && !initialized) {
      ...
      setAmountRupees(paiseToRupees(expense.totalPaise).toString());
      setCategory(expense.category);
      setDate(new Date(expense.date).toISOString().split('T')[0]);
      setSplitType(expense.splitType);

      if (expense.contributors.length > 0) {
        setPayerId(expense.contributors[0].userId);
      }
      ...
      if (expense.splitType === 'EQUAL') {
        ...
        expense.splits.forEach(s => {
          amounts[s.userId] = paiseToRupees(s.amountPaise).toString();
        });
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => updateGroupExpense(houseId, expenseId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['groupExpenses', houseId] });
      queryClient.invalidateQueries({ queryKey: ['groupBalances', houseId] });
      queryClient.invalidateQueries({ queryKey: ['groupExpense', houseId, expenseId] });
  ...
  if (groupLoading || expenseLoading) return <div className="skeleton" style={{ height: 400 }} />;

  if (expense && user && expense.createdById !== user.id) {
    return (
      <div className="card empty-state" style={{ padding: 48 }}>
        <h3 className="empty-state-title">Not Authorized</h3>
        <p className="empty-state-description">You can only edit expenses that you created.</p>
        <Link href={`/houses/${houseId}/expenses`} className="btn btn-primary btn-sm" style={{ marginTop: 16 }}>
          Back to Expenses
        </Link>
      </div>
    );
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
      <div className="page-header">
        ...
        <Link href={`/houses/${houseId}/expenses`} className="btn btn-secondary">Cancel</Link>
      </div>
          ...
          <div className="animate-fade-in">
            ...
            <div style={{ marginTop: 32, display: 'flex', justifyContent: 'space-between' }}>
              ...
              <button className="btn btn-primary btn-lg" onClick={handleSubmit} disabled={mutation.isPending}>
                {mutation.isPending ? 'Saving...' : 'Update Expense'}
              </button>
            </div>
          </div>
        )}
